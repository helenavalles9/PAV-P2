...
typedef enum {ST_UNDEF=0, ST_SILENCE, ST_VOICE, ST_INIT, ST_MVOICE, ST_MSILENCE} VAD_STATE;
...
typedef struct {
  VAD_STATE state;
  float sampling_rate;
  unsigned int frame_length;  /* in samples*/
  ...
  double initFrames;     /*Number of frames used to initialize*/
} VAD_DATA;
